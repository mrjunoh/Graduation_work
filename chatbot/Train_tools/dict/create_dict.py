...
add = pd.read_csv('데이터/data_to_csv/추가데이터.csv')
add_1 = pd.read_csv('데이터/data_to_csv/추가_1.csv')
...
text5 = list(add['query']) #추가데이터
text6 = list(add['query']) #추가_1

#리스트 다 더하기
corpus_data = text1 + text2 + text3 + text4 + text5 + text6
...
#단어 사전에 포함되는 단어의 수를 가장 많이 등장하는 20만개의 단어로 지정함
tokenizer = preprocessing.text.Tokenizer(oov_token='OOV', num_words = 200000)
